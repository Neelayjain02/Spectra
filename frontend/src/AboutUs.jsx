    ...
    <AnimatePresence>
      ...
      >
        ...
        >
          ...
          <p className="text-center text-gray-300 mb-2">{member.role}</p>
...
export default function AboutUs() {
  ...
  return (
    ...
    >
      ...
      {/* Content */}
      <motion.div
        className="absolute inset-0 z-10 flex flex-col items-center justify-center px-8 gap-12 max-w-7xl mx-auto"
      ...
      >
        {/* About the Challenge */}
        <motion.div
          className="bg-black/50 backdrop-blur-md p-8 rounded-3xl border border-cyan-400/30 shadow-[0_0_20px_rgba(34,211,238,0.2)] w-full"
          whileHover={{ scale: 1.02 }}
        >
          <h2 className="text-3xl md:text-4xl font-extrabold text-transparent bg-clip-text bg-gradient-to-r from-cyan-400 via-indigo-300 to-purple-500 mb-4">
            ABOUT THE CHALLENGE
          </h2>
          <p className="text-gray-200 text-base md:text-lg leading-relaxed">
            The challenge focuses on developing an AI/ML model trained on NASA’s open exoplanet datasets to identify new exoplanets and create a user-friendly web interface for interaction. It automates analysis of complex astronomical data—traditionally manual—making discovery faster, more accurate, and accessible to researchers and enthusiasts.
          </p>
        </motion.div>

        {/* About Us Team Members */}
        <motion.div className="flex flex-col md:flex-row gap-12 w-full">
          {teamMembers.map((member) => (
            <motion.div
              key={member.name}
              className="flex-1 flex flex-col items-center bg-black/50 backdrop-blur-md p-6 rounded-3xl border border-cyan-400/30 shadow-[0_0_20px_rgba(34,211,238,0.2)] cursor-pointer hover:scale-105 transition-transform duration-300"
              onClick={() => setSelectedMember(member)}
            >
              <img
                src={member.photo}
                alt={member.name}
                className="w-32 h-32 rounded-full object-cover border-4 border-cyan-400 mb-4 hover:shadow-[0_0_30px_#00ffff] transition-shadow duration-300"
              />
        ...
        {/* Team Intro Line */}
        <motion.p
          className="mt-6 text-center text-lg md:text-xl text-cyan-300 font-medium max-w-3xl mx-auto"
          initial={{ opacity: 0, y: 20 }}
          animate={{ opacity: 1, y: 0 }}
          transition={{ delay: 0.5, duration: 1 }}
        >
          We are Team SPECTRA — a duo of passionate innovators exploring the intersection of artificial intelligence and space science.
        </motion.p>
      </motion.div>
